...
def get_routes(config: ApplicationConfig, context: Context) -> Tuple:
    return (
        ...
        endpoints.UserEndpoint(
            config, context, uri='/user/<uid:int>', methods=['PATCH', 'GET'], auth_required=True,
        ),
        endpoints.AllUserEndpoint(
            config, context, uri='/user/all', methods=['GET'], auth_required=True,
        ),
        endpoints.MessageEndpoint(
            config, context, uri='/user/<uid:int>/msg', methods=['POST', 'GET'], auth_required=True,
        ),
        endpoints.UserMessageEndpoint(
            config, context, uri='/user/<uid:int>/msg/<mid:int>',
            methods=['GET', 'PATCH', 'DELETE'], auth_required=True,
        ),
    )
